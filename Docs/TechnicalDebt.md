...
**문제**: 컴포넌트 조합 방식으로 인해 데이터와 로직이 분산되어 있어 일괄 관리 어려움

**영향도**: 🟡 중 | **우선순위**: P2

**현재 구조**: 14종 적의 스탯이 프리팹 내 여러 컴포넌트에 분산 (EnemyShip, Damageable, Rigidbody2D, Impactable, EnemyMover 등)

**문제점**:
- MVC 패턴 적용 시도했으나 CSV 데이터를 프리팹에 수동 반영 필요
- 컴포넌트 조합 특성상 데이터와 동작 스크립트 분리 어려움
- 밸런싱 수정 시 CSV → 프리팹 동기화 비효율적

**해결 옵션**:
- **ScriptableObject 기반**: EnemyData에 스탯 중앙화, CSV → SO 임포터 작성
- **하이브리드**: 프리팹 유지하되 런타임에 데이터 오버라이드

**참고**: [EnemyStats.csv](../Assets/Data/Balancing/EnemyStats.csv), [EnemyShip.cs](../Assets/Scripts/SpaceShips/EnemyShip.cs)

---

### 2.5 스폰 시스템 동시성 문제 ✅ 해결 완료
